...
SKIP: {
    ...
    skip "Can't find temporary directory", 26
    ...
    my $thawed = File::Monitor->new( { base => $test_dir } );
    $thawed->watch( { name => $test_dir, recurse => 1 } );

    my @changed = $thawed->scan;
    ...
    for my $test ( @schedule ) {
        ...
        my $frozen = eval { freeze $thawed };
        ok !$@, "$name: freeze OK" or diag "Error from freeze: $@";
          
        my $thawed = eval { thaw $frozen };
        ok !$@, "$name: thaw OK" or diag "Error from thaw: $@";
        isa_ok $thawed, 'File::Monitor';
        
        $thawed->base( $new_dir );
        $test_dir = $new_dir;

        is $thawed->base, $test_dir, "$name: monitor relocated";

        my %expect = ();

        # Get the expected changes
        for my $mode ( [ 'add', 'files_created' ], [ 'rm', 'files_deleted' ] ) {
            my ( $act, $key ) = @$mode;
